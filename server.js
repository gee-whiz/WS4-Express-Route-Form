import path from "path"; // Load Node.js path module for safe file paths
import express from "express"; // Load Express framework
import axios from "axios"; // Load Axios HTTP client
...
app.get("/", (_, res) => { // Handle GET request to root path
  res.sendFile(
...
router.get(
  "/time",
  (_, res) => res.json({ now: new Date().toISOString() })
